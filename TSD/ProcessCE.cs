...
namespace Terranova.API
{
    ...
    public class ProcessInfo
    {
        ...
        internal ProcessInfo(IntPtr pid, int threadcount, int baseaddress, int parentid)
        {
            ...
            _exeName = System.IO.Path.GetFileName(_fullPath);
        }

        internal ProcessInfo(IntPtr pid, int threadcount, int baseaddress, int parentid, string exename)
        {
            _pid = pid;
            _threadCount = threadcount;
            _baseAddress = baseaddress;
    ...
    public static class ProcessCE
    {
        ...
        /// <returns>Returns true is the process is running.</returns>
        /// <exception cref="Win32Exception">Thrown when taking a system snapshot fails.</exception>
        public static bool IsRunningSimple(string exeName)
        {
            ProcessInfo[] list = GetProcesses();
            List<string> sExeFiles = new List<string>();
            foreach (ProcessInfo pi in list)
                sExeFiles.Add(System.IO.Path.GetFileName(pi.FullPath).ToLower());
            if (sExeFiles.Contains(exeName.ToLower()))
                return true;
            else
                return false;
        }
        ...
        public static IntPtr FindProcessPIDShortname(string shortname)
        {
            ...
            try
            {
                PROCESSENTRY processentry = new PROCESSENTRY();
                processentry.dwSize = (uint)Marshal.SizeOf(processentry);
                StringBuilder exename = new StringBuilder(255);
                ...
                while (retval == 1)
                {
                    ...
                    // Note: If first parameter is IntPtr.Zero it returns the path to the current process.
                    //GetModuleFileName(pid, fullexepath, fullexepath.Capacity);
                    exename = new StringBuilder(processentry.szExeFile);
                    if (exename.ToString().ToLower() == shortname)
                        return pid;
        ...
        public static bool quitWindow(string winTitle)
        {
            IntPtr hwnd = FindWindowByCaption(IntPtr.Zero, winTitle);
            if (hwnd.ToInt32() != INVALID_HANDLE_VALUE)
            {
                SendMessage(hwnd, (int)WM_QUIT, IntPtr.Zero, IntPtr.Zero);
                return true;
            }
            return false;
        }
        ...
        [DllImport("coredll.dll", SetLastError=true)]
        private static extern IntPtr FindWindow(string szClass, String szTitle);
